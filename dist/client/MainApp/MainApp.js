...
const MainPage_BoardPiecesItem = React.memo(function MainPage_BoardPiecesItem(props) {
    ...
    const styles = elProps(pathTo('BoardPieces.Styles')).width(100 / Columns + '%').border('1px solid blue').backgroundColor(If(Or($itemId == FromPosition, $itemId == ToPosition), 'orange', 'white')).aspectRatio('1').boxSizing('border-box').position('relative').props

    return React.createElement(ItemSetItem, {path: props.path, item: $item, itemId: $itemId, index: $index, onClick, canDragItem, styles},
        React.createElement(Icon, elProps(pathTo('PieceIcon')).iconName(If($item == 'obstacle', 'coronavirus', () => If($item == 'player', 'sentiment_very_satisfied'))).styles(elProps(pathTo('PieceIcon.Styles')).position('absolute').top('50%').left('50%').translate('-50% -50%').color(If($item == 'obstacle', 'red', () => If($item == 'player', 'green'))).fontSize('3em').width('60%').backgroundColor(If($item == 'obstacle', 'green')).height('60%').paddingTop('0%').props).props),
    )
})
...
function MainPage(props) {
    ...
    const {Floor, And, IsNull, ForEach, Range, Or, Not, If, RandomListFrom, RandomFrom, ListContains, WithUpdates, ItemAt, Select, Ceiling} = Elemento.globalFunctions
    ...
    const PointsAllowed = _state.setObject(pathTo('PointsAllowed'), new Calculation.State(stateProps(pathTo('PointsAllowed')).value(50).props))
    ...
    const DoPlayerObstacleMove = _state.setObject(pathTo('DoPlayerObstacleMove'), React.useCallback(wrapFn(pathTo('DoPlayerObstacleMove'), 'calculation', (position) => {
        ...
        return Set(PointsUsed, PointsUsed + 5)
    }), [ObstacleMove, RandomPosition, PointsUsed]))
    ...
    return React.createElement(Page, elProps(props.path).props,
        ...
        React.createElement(Dialog, elProps(pathTo('Instructions')).layout('vertical').showCloseButton(true).styles(elProps(pathTo('Instructions.Styles')).padding('2em').props).props,
            React.createElement(TextElement, elProps(pathTo('InstructionsText')).allowHtml(true).content(`Move your piece - the smiley face - to the top row of the board, while keeping as many points as possible.
...
Click on an obstacle to make it move somewhere else - this costs 5 points.
...
You have 3 minutes to complete as many boards as you can.`).props),
...
Click on empty adjacent squares to move your piece, or click on an obstacle to move it.
